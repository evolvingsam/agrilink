...
9. [Market Matching, Orders & Payments](#9-market-matching-orders--payments)
10. [Logistics & Routing Endpoints](#10-logistics--routing-endpoints)
11. [Payments (Farmer Payout) Endpoints](#11-payments-farmer-payout-endpoints)
12. [Market Trends & Demand Forecasting](#12-market-trends--demand-forecasting-endpoints)
13. [Future Endpoints (Coming Soon)](#13-future-endpoints-coming-soon)
---
...
2. **Place Order** → specify crop, quantity, max price, and minimum grade.
3. **AI Auto-Matching** → matching engine runs immediately on order creation and matches available listings.
4. **Pay for Order** → deducts from demo wallet (₦500,000 default balance). Animated confirmation flow.
5. **My Orders Dashboard** → view status, expand to see matched farmer listings, cancel unpaid orders.
...
### Base URL
```
http://127.0.0.1:8000/api/   ← Local development (port 8000)
```
> ⚠️ **Port is 8000** — not 8001. Set this in your `.env` as `VITE_API_BASE_URL=http://127.0.0.1:8000/api`.

*Production base URL will be provided separately once deployed. Set `DATABASE_URL` env var to switch from SQLite to the Postgres instance on Render.*
...
{
  "count": 120,
  "next": "http://127.0.0.1:8000/api/produce/listings/?page=2",
  "previous": null,
...
> 🎁 Every new user is automatically given a **₦500,000 demo wallet balance**.
...
### `GET /api/auth/me/`
Get the currently authenticated user's full profile, **including their wallet balance**.
...
```json
{
  ...
  "role": "buyer",
  "phone": "08012345678",
  "wallet_balance": "493500.00",
  "farmer_profile": null,
  "buyer_profile": {
    "business_name": "GodsOwn Ventures",
    "location": "Lagos",
    "verified_status": false
  }
}
```
> `wallet_balance` is read-only from the API. It is deducted server-side when `POST /orders/{id}/pay/` is called.
> `farmer_profile` is `null` for buyers and dispatchers. `buyer_profile` is `null` for farmers and dispatchers.
> **Frontend task:** Call this immediately after login to determine `role` and render the correct dashboard. Display `wallet_balance` in the buyer navbar/header.
  ...
  "results": [
    {
      ...
      "status": "graded",
      "quality_grade": "A",
      "photo": "http://127.0.0.1:8000/media/produce/photos/tomato_abc.jpg",
...
6. **Wallet Balance (Buyer)**: Display the buyer's `wallet_balance` from `/auth/me/` prominently in the header/navbar. Update it immediately after a successful payment (the `/orders/{id}/pay/` response returns `new_wallet_balance`).

7. **Payment Flow UX**: Use a 3-step animation for payments:
   - Step 1: Confirmation modal showing cost breakdown.
   - Step 2: Animated "Processing Payment…" spinner (~2 seconds).
   - Step 3: ✅ success tick → auto-close modal → slide-in toast notification.

8. **Offline Awareness**: Display a banner when the device is offline. Queue chat messages and listing creates locally and sync when reconnected. TanStack Query's `useMutation` with offline support handles this well.

---

## 9. Market Matching, Orders & Payments

**Prefix:** `/api/orders/`

> ⚡ **Auto-matching on order creation:** When a buyer POSTs a new order, the matching engine runs immediately in the same request, so matches are typically available within seconds — no cron job or polling needed for demos.

---

### `POST /api/orders/`
Buyer places an order specifying what produce they need.
...
| Field | Description |
|---|---|
| `crop_type` | ID from `GET /produce/crops/` |
| `quantity_kg` | How much produce they need |
| `max_price_per_kg` | Maximum they are willing to pay per kg |
| `required_grade` | `"A"`, `"B"`, or `"C"` |
...
```json
{
  ...
  "required_grade": "A",
  "status": "open",
  "created_at": "2026-07-24T12:27:00Z"
}
```
...
**Response `200 OK` (paginated):** Array of order objects (same shape as above).
...
**Pay for an order.** Deducts `quantity_kg × max_price_per_kg` from the buyer's wallet and advances the order status to `matched` (shown as "Processing" in the UI).

**Auth required:** ✅ Yes (Order owner only)

**Request Body:** *(empty — cost is calculated server-side)*

**Response `200 OK`:**
```json
{
  "success": true,
  "message": "Payment of ₦30,000.00 processed successfully!",
  "new_wallet_balance": 470000.00,
...
```json
{
  "error": "Insufficient wallet balance. You need ₦30,000.00 but have ₦5,000.00."
}
```
...
```json
{
  "error": "Order is already matched. Only OPEN orders can be paid."
}
```

> **Frontend task:** After calling this, update the buyer's displayed wallet balance using `new_wallet_balance` from the response — do NOT re-fetch `/auth/me/` as this causes an extra round-trip. Store it in local state.

---

### `DELETE /api/orders/{id}/cancel/`
Cancel an unpaid (`open`) order. No refund is issued as no payment was made.
...
```json
{
  "error": "Only OPEN (unpaid) orders can be cancelled."
}
```

---

### `GET /api/orders/matching/results/{order_id}/`
View the AI-matched farmer listings for a specific order.
  ...
  "results": [
    {
      ...
      "listing_id": 12,
      "order_id": 3,
      "farmer_name": "mrv_farmer",
      "buyer_name": "godsown",
      "price_per_kg": "450.00",
      "quality_grade": "A",
      "match_score": 0.95,
      "status": "pending_delivery",
      "created_at": "2026-07-24T12:27:05Z"
    }
  ]
}
```
...
### `POST /api/orders/matching/run/`
*(Admin Only)* Manually trigger the matching cycle algorithm.
...
## 11. Payments (Farmer Payout) Endpoints

**Prefix:** `/api/payments/`

> These endpoints manage **farmer payouts** — different from the buyer wallet payment in Section 9. After a delivery is confirmed, admin triggers this to simulate paying the farmer.
...
### Push Notifications
```
POST  /api/notifications/register-device/              → Register FCM/APNS device token
